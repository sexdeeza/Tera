...
function start() {
	if (im.getPlayer().getMap().getId() != 211041400) {
		im.getClient().getSession().write(Packages.tools.packet.MaplePacketCreator.serverNotice(6, "未到達亡者之林Ⅳ，無法使用結界的圖騰"));
		im.dispose();
		return;
		}
		...
		im.getClient().getSession().write(Packages.tools.packet.MaplePacketCreator.serverNotice(6, "結界的圖騰發出耀眼的光芒，開始燃起火焰，刻在圖騰上的亞凱斯特的結界魔法發動了"));
		im.dispose();
}